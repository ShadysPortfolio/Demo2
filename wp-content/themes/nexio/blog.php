...
if ( function_exists( 'nexio_elementor_is_preview' ) && nexio_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			nexio_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	nexio_new_main_query(
		array(
			'post_type'      => nexio_get_theme_option( 'post_type' ),
			'category'       => nexio_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'nexio_filter_get_template_part', nexio_blog_archive_get_template() ) );
}
