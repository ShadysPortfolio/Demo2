...
if ( ! function_exists( 'nexio_storage_get' ) ) {
	/**
	 ...
	 */
	function nexio_storage_get( $var_name, $default = '' ) {
		global $NEXIO_STORAGE;
		return isset( $NEXIO_STORAGE[ $var_name ] ) ? $NEXIO_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'nexio_storage_set' ) ) {
	/**
	 ...
	 */
	function nexio_storage_set( $var_name, $value ) {
		global $NEXIO_STORAGE;
		$NEXIO_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'nexio_storage_empty' ) ) {
	/**
	 ...
	 */
	function nexio_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $NEXIO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $NEXIO_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $NEXIO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'nexio_storage_isset' ) ) {
	/**
	 ...
	 */
	function nexio_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $NEXIO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $NEXIO_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $NEXIO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'nexio_storage_unset' ) ) {
	/**
	 ...
	 */
	function nexio_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $NEXIO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $NEXIO_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $NEXIO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'nexio_storage_inc' ) ) {
	/**
	 ...
	 */
	function nexio_storage_inc( $var_name, $value = 1 ) {
		global $NEXIO_STORAGE;
		if ( empty( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = 0;
		}
		$NEXIO_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'nexio_storage_concat' ) ) {
	/**
	 ...
	 */
	function nexio_storage_concat( $var_name, $value ) {
		global $NEXIO_STORAGE;
		if ( empty( $NEXIO_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'nexio_storage_get_array' ) ) {
	/**
	 ...
	 */
	function nexio_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $NEXIO_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $NEXIO_STORAGE[ $var_name ][ $key ] ) ? $NEXIO_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'nexio_storage_set_array' ) ) {
	/**
	 ...
	 */
	function nexio_storage_set_array( $var_name, $key, $value ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$NEXIO_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'nexio_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function nexio_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $NEXIO_STORAGE[ $var_name ][ $key ] ) ) {
			$NEXIO_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$NEXIO_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$NEXIO_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'nexio_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function nexio_storage_merge_array( $var_name, $key, $value ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$NEXIO_STORAGE[ $var_name ] = array_merge( $NEXIO_STORAGE[ $var_name ], $value );
		} else {
			$NEXIO_STORAGE[ $var_name ][ $key ] = array_merge( $NEXIO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'nexio_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function nexio_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			nexio_array_insert_after( $NEXIO_STORAGE[ $var_name ], $after, $key );
		} else {
			nexio_array_insert_after( $NEXIO_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'nexio_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function nexio_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			nexio_array_insert_before( $NEXIO_STORAGE[ $var_name ], $before, $key );
		} else {
			nexio_array_insert_before( $NEXIO_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'nexio_storage_push_array' ) ) {
	/**
	 ...
	 */
	function nexio_storage_push_array( $var_name, $key, $value ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $NEXIO_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $NEXIO_STORAGE[ $var_name ][ $key ] ) ) {
				$NEXIO_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $NEXIO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'nexio_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function nexio_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $NEXIO_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $NEXIO_STORAGE[ $var_name ] ) && is_array( $NEXIO_STORAGE[ $var_name ] ) && count( $NEXIO_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $NEXIO_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $NEXIO_STORAGE[ $var_name ][ $key ] ) && is_array( $NEXIO_STORAGE[ $var_name ][ $key ] ) && count( $NEXIO_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $NEXIO_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'nexio_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function nexio_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $NEXIO_STORAGE[ $var_name ][ $key ] ) ) {
			$NEXIO_STORAGE[ $var_name ][ $key ] = 0;
		}
		$NEXIO_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'nexio_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function nexio_storage_concat_array( $var_name, $key, $value ) {
		global $NEXIO_STORAGE;
		if ( ! isset( $NEXIO_STORAGE[ $var_name ] ) ) {
			$NEXIO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $NEXIO_STORAGE[ $var_name ][ $key ] ) ) {
			$NEXIO_STORAGE[ $var_name ][ $key ] = '';
		}
		$NEXIO_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'nexio_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function nexio_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $NEXIO_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $NEXIO_STORAGE[ $var_name ] ) ? $NEXIO_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $NEXIO_STORAGE[ $var_name ] ) ? $NEXIO_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'nexio_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function nexio_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $NEXIO_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $NEXIO_STORAGE[ $var_name ]->$prop ) ? $NEXIO_STORAGE[ $var_name ]->$prop : $default;
	}
}
