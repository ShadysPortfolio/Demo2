...
if ( ! function_exists( 'nexio_wp_theme_setup' ) ) {

	add_action( 'after_setup_theme', 'nexio_wp_theme_setup' );
	...
	function nexio_wp_theme_setup() {
		...
		add_filter( 'post_type_archive_link', 'nexio_get_template_page_link', 10, 2 );
	}
}


if ( ! function_exists( 'nexio_is_preview' ) ) {
	/**
	 ...
	 */
	function nexio_is_preview( $builder = 'any' ) {
		return ( in_array( $builder, array( 'any', 'elm', 'elementor' ) ) && function_exists( 'nexio_elementor_is_preview' ) && nexio_elementor_is_preview() )
				||
				( in_array( $builder, array( 'any', 'gb', 'gutenberg' ) ) && function_exists( 'nexio_gutenberg_is_preview' ) && nexio_gutenberg_is_preview() );
	}
}


if ( ! function_exists( 'nexio_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function nexio_get_wp_template_hooks() {
		return apply_filters( 'nexio_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'nexio_detect_blog_mode' ) ) {
	/**
	 ...
	 * 
	 * For example:
	 *   $pos = nexio_get_theme_option( 'sidebar_position' );
	 * 
	 ...
	 */
	function nexio_detect_blog_mode() {
		if ( is_front_page() && ! is_home() ) {
			$mode = 'front';
		} elseif ( is_home() ) {
			$mode = 'home';     // Specify 'blog' if you don't need a separate options for the homepage
		} elseif ( nexio_is_single() ) {
			$mode = 'post';
		} elseif ( is_page() && ! nexio_storage_isset( 'blog_archive' ) ) {
			$mode = 'page';
		} elseif ( is_category() ) {
			$mode = 'category';
		} elseif ( is_tag() ) {
		...
		return apply_filters( 'nexio_filter_detect_blog_mode', $mode );
	}
}

if ( ! function_exists( 'nexio_is_blog_mode_custom' ) ) {
	/**
	 ...
	 */
	function nexio_is_blog_mode_custom() {
		return ! in_array( nexio_storage_get( 'blog_mode' ), apply_filters( 'nexio_filter_blog_mode_standard', array(
...
if ( ! function_exists( 'nexio_get_current_mode_image' ) ) {
	/**
	 ...
	 */
	function nexio_get_current_mode_image( $default = '' ) {
		if ( is_category() || is_tax() ) {
			$img = nexio_get_term_image();
			if ( '' != $img ) {
				$default = $img;
			}
		} elseif ( nexio_is_singular() || nexio_storage_isset( 'blog_archive' ) ) {
			if ( is_home() ) {
		...
		return nexio_clear_thumb_size( $default );
	}
}

if ( ! function_exists( 'nexio_blog_archive_start' ) ) {
	/**
	 ...
	 */
	function nexio_blog_archive_start() {
		$main_post = nexio_storage_get( 'blog_archive_template_post' );
		if ( is_object( $main_post ) ) {
			...
			$nexio_content            = '';
			$nexio_blog_archive_mask  = '%%CONTENT%%';
			$nexio_blog_archive_subst = sprintf( '<div class="blog_archive">%s</div>', $nexio_blog_archive_mask );
			$nexio_content            = apply_filters( 'the_content', get_the_content() );
			...
			if ( '' != $nexio_content ) {
				$nexio_pos = stripos( $nexio_content, $nexio_blog_archive_mask );
				if ( false !== $nexio_pos ) {
					$nexio_content = preg_replace( '/(\<p\>\s*)?' . $nexio_blog_archive_mask . '(\s*\<\/p\>)/i', $nexio_blog_archive_subst, $nexio_content );
				} else {
					$nexio_content .= $nexio_blog_archive_subst;
				}
				$nexio_content = explode( $nexio_blog_archive_mask, $nexio_content );
				// Display first part
				nexio_show_layout( apply_filters( 'nexio_filter_blog_archive_start', $nexio_content[0] ) );
				// And store second part
				nexio_storage_set( 'blog_archive_end', $nexio_content[1] );
			}
...
if ( ! function_exists( 'nexio_blog_archive_end' ) ) {
	/**
	 * Display the end blog archive template.
	 * 
	 * Output a second part (stored in the previous call of nexio_blog_archive_start).
	 */
	function nexio_blog_archive_end() {
		$html = nexio_storage_get( 'blog_archive_end' );
		if ( '' != $html ) {
			// Display second part of template content
			nexio_show_layout( apply_filters( 'nexio_filter_blog_archive_end', $html ) );
		}
	}
}

if ( ! function_exists( 'nexio_blog_archive_get_template' ) ) {
	/**
	 ...
	 */
	function nexio_blog_archive_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = nexio_get_theme_option( 'blog_style' );
		}
		$parts   = explode( '_', $blog_style );
		$archive = 'index';
		if ( nexio_storage_isset( 'blog_styles', $parts[0], 'archive' ) ) {
			$archive = nexio_storage_get_array( 'blog_styles', $parts[0], 'archive' );
		}
		return apply_filters( 'nexio_filter_blog_archive_template', $archive, $blog_style );
	}
}

if ( ! function_exists( 'nexio_blog_item_get_template' ) ) {
	/**
	 ...
	 *                           or an empty string if not detected.
	 */
	function nexio_blog_item_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = nexio_get_theme_option( 'blog_style' );
		}
		$parts = explode( '_', $blog_style );
		...
		} elseif ( nexio_storage_isset( 'blog_styles', $parts[0], 'item' ) ) {
			$item = nexio_storage_get_array( 'blog_styles', $parts[0], 'item' );
		} else {
...
if ( ! function_exists( 'nexio_get_post_id' ) ) {
	/**
	 ...
	 */
	function nexio_get_post_id( $args = array() ) {
		$args  = array_merge(
			array(
				'posts_per_page' => 1,
			), $args
...
if ( ! function_exists( 'nexio_get_post_content' ) ) {
	/**
	 ...
	 */
	function nexio_get_post_content( $apply_filters = false ) {
		global $post;
...
if ( ! function_exists( 'nexio_filter_post_content' ) ) {
	/**
	 ...
	 */
	function nexio_filter_post_content( $content ) {
		$content = apply_filters( 'nexio_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'nexio_get_template_page_id' ) ) {
	/**
	 ...
	 */
	function nexio_get_template_page_id( $args = array() ) {
		$args   = array_merge(
			array(
				'template'   => 'blog.php',
				'post_type'  => 'post',
		...
		if ( '' !== $args['parent_cat'] ) {
			$q_args['meta_query'][] = array(
				'key'     => 'nexio_options_parent_cat',
				'value'   => $args['parent_cat'] > 0 ? $args['parent_cat'] : 1,
				'compare' => $args['parent_cat'] > 0 ? '=' : '<',
			);
		}
		return nexio_get_post_id( $q_args );
	}
}

if ( ! function_exists( 'nexio_get_template_page_link' ) ) {
	/**
	 ...
	 * 
	 * Handler of the add_filter('post_type_archive_link', 'nexio_get_template_page_link', 10, 2 );
	 * 
	 ...
	 */
	function nexio_get_template_page_link( $link = '', $post_type = '' ) {
		if ( ! empty( $post_type ) ) {
			$id = nexio_get_template_page_id(
				array(
					'post_type'  => $post_type,
					'parent_cat' => 0,
				)
...
if ( ! function_exists( 'nexio_get_posts_archive_template' ) ) {

	add_filter( 'archive_template', 'nexio_get_posts_archive_template', 100 );
	 ...
	 */
	function nexio_get_posts_archive_template( $template ) {
		if ( nexio_get_theme_option( 'use_blog_archive_pages' ) ) {
			if ( is_post_type_archive() ) {
				$obj = get_queried_object();
				if ( ! empty( $obj->name ) ) {
					$templates = get_option( 'nexio_blog_archive_templates' );
					if ( ! empty( $templates[ $obj->name ] ) ) {
						$template = nexio_redirect_to_archive_template( $template, $templates[ $obj->name ] );
					}
				}
			} else {
				$template = nexio_get_tax_archive_template( $template );
			}
		}
		return $template;
	}	
...
if ( ! function_exists( 'nexio_get_tax_archive_template' ) ) {

	add_filter( 'category_template', 'nexio_get_tax_archive_template', 100 );
	add_filter( 'taxonomy_template', 'nexio_get_tax_archive_template', 100 );
	 ...
	 * @return string          A filtered path to the template
	 */
	function nexio_get_tax_archive_template( $template ) {
		if ( nexio_get_theme_option( 'use_blog_archive_pages' ) && ( is_category() || is_tag() || is_tax() ) ) {
			$obj = get_queried_object();
			global $wp_query;
			...
			if ( $pt && $tax && $term ) {
				$pt_tax  = nexio_get_post_type_taxonomy( $pt );
				if ( $pt_tax == $tax ) {
					$templates = get_option( 'nexio_blog_archive_templates' );
					if ( ! empty( $templates[ "{$pt}_{$tax}_{$term}" ] ) ) {
						$template = nexio_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$term}" ] );
					} else {
						$found = false;
						do {
							...
							if ( ! empty( $templates[ "{$pt}_{$tax}_{$parent}" ] ) ) {
								$template = nexio_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$parent}" ], $term );
								$found = true;
								break;
							} else {
						...
						if ( ! $found && ! empty( $templates[ "{$pt}" ] ) ) {
							$template = nexio_redirect_to_archive_template( $template, $templates[ "{$pt}" ], $term );
						}
					}
				}
			}
...
if ( ! function_exists( 'nexio_redirect_to_archive_template' ) ) {
	/**
	 ...
	 */
	function nexio_redirect_to_archive_template( $template, $page_id, $term = false ) {
		// Store page number
		...
		// Load page options
		nexio_override_theme_options( null, $page_id );
		// Override parent category
		if ( $term > 0 ) {
			nexio_storage_set_array( 'options_meta', 'parent_cat', $term );
		}
		return nexio_get_file_dir( 'blog.php' );
	}
}

if ( ! function_exists( 'nexio_save_archive_template' ) ) {

	add_action( 'save_post', 'nexio_save_archive_template', 11 );
	 ...
	 */
	function nexio_save_archive_template( $post_id ) {
		// verify nonce
		if ( ! wp_verify_nonce( nexio_get_value_gp( 'override_options_nonce' ), admin_url() ) ) {
			return $post_id;
		}
		...
		if ( 'page' == $post_type ) {
			...
			if ( 'blog.php' == $page_template ) {
				...
				$tax  = nexio_get_post_type_taxonomy( $pt );
				$term = isset( $meta['parent_cat'] ) ? $meta['parent_cat'] : 0;
				$templates = nexio_array_delete_by_value( $templates, $post_id );
				$templates[ $pt . ( $term > 0 ? "_{$tax}_{$term}" : '' ) ] = $post_id;
			} else {
				$templates = nexio_array_delete_by_value( $templates, $post_id );
			}
			update_option( 'nexio_blog_archive_templates', $templates );
		}
	}
}

// Delete page from templates
if ( ! function_exists( 'nexio_delete_archive_template' ) ) {

	add_action( 'delete_post', 'nexio_delete_archive_template', 11 );
	 ...
	 */
	function nexio_delete_archive_template( $post_id ) {
		$templates = get_option( 'nexio_blog_archive_templates' );
		if ( is_array( $templates ) ) {
			$templates = nexio_array_delete_by_value( $templates, $post_id );
			update_option( 'nexio_blog_archive_templates', $templates );
...
if ( ! function_exists( 'nexio_get_hash_link' ) ) {
	 ...
	 */
	function nexio_get_hash_link( $hash ) {
...
if ( ! function_exists( 'nexio_get_current_url' ) ) {
	/**
	 * Return an URL to the current page.
	 * 
	 * @return string  A current page URL.
	 */
	function nexio_get_current_url() {
		return add_query_arg( array() );
	}
}

if ( ! function_exists( 'nexio_check_url' ) ) {
	/**
...
if ( ! function_exists( 'nexio_remove_macros_from_site_title' ) ) {
	 ...
	 */
	function nexio_remove_macros_from_site_title( $title ) {
		if ( is_array( $title ) ) {
			foreach ( $title as $k => $v ) {
				$title[ $k ] = nexio_remove_macros( $v );
			}
		} else {
			$title = nexio_remove_macros( $title );
		}
		return $title;
	}
}

if ( ! function_exists( 'nexio_get_blog_title' ) ) {
	/**
	 * Return a blog title for the current blog mode.
	 *
	 * @return string  A blog title.
	 */
	function nexio_get_blog_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'nexio' );
		} elseif ( is_home() ) {
			$title = esc_html__( 'All Posts', 'nexio' );
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author page: %s', 'nexio' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'nexio' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'nexio' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'nexio' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'nexio' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'nexio' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'nexio' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'nexio' ), get_the_title() );
		} elseif ( nexio_is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'nexio_filter_get_blog_title', $title );
	}
}

if ( ! function_exists( 'nexio_get_post_categories' ) ) {
	/**
	...
	function nexio_query_add_posts_and_cats( $args, $ids = '', $post_type = '', $cat = '', $taxonomy = '' ) {
		if ( ! empty( $ids ) ) {
			...
			if ( ! empty( $cat ) ) {
				...
				if ( empty( $taxonomy ) ) {
					$taxonomy = nexio_get_post_type_taxonomy( $post_type );
				}
...
if ( ! function_exists( 'nexio_query_add_filters' ) ) {
	/**
...
if ( ! function_exists( 'nexio_create_widgets_area' ) ) {
	...
	function nexio_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = nexio_get_theme_option( $name );
...
if ( ! function_exists( 'nexio_get_content_width' ) ) {
	...
	function nexio_get_content_width() {
		$pg_width = apply_filters( 'nexio_filter_content_width', nexio_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( nexio_sidebar_present() ) {
			...
			if ( 'default' == $nexio_sidebar_type ) {
				...
				if ( is_active_sidebar( $nexio_sidebar_name ) ) {
					$sb_width = nexio_get_theme_option( 'sidebar_width' );
					$sb_gap   = nexio_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = nexio_get_theme_option( 'sidebar_width' );
				$sb_gap   = nexio_get_theme_option( 'sidebar_gap' );
...
------------------------------------------------------------------------------------- */
if ( ! function_exists( 'nexio_enqueue_styles' ) ) {
	/**
	 * Enqueue a list of styles
	 *
	 * @param array $list - styles to enqueue
	 */
	function nexio_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = nexio_get_file_url( $data['src'] );
				if ( $lib_url ) {
					wp_enqueue_style(
						$handle,
						$lib_url,
						...
						! empty( $data['media'] ) ? nexio_media_for_load_css_responsive( str_replace( '_', '-', $sc ), $data['media'] ) : 'all'
					);
				}
			}
		}
...
if ( ! function_exists( 'nexio_enqueue_scripts' ) ) {
	/**
	 * Enqueue a list of scripts
	 *
	 * @param array $list - scripts to enqueue
	 */
	function nexio_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = nexio_get_file_url( $data['src'] );
				if ( $lib_url ) {
...
if ( ! function_exists( 'nexio_enqueue_optimized' ) ) {
	/**
	 ...
	 */
	function nexio_enqueue_optimized( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && nexio_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
			...
			if ( ! isset( $args['need'] ) || $args['need'] ) {
				if ( ! empty( $args['css'] ) ) {
					nexio_enqueue_styles( $args['css'], $sc );
				}
				if ( ! empty( $args['js'] ) ) {
					nexio_enqueue_scripts( $args['js'], $sc );
				}
...
if ( ! function_exists( 'nexio_enqueue_optimized_responsive' ) ) {
	/**
	 ...
	 * @param array $args - arguments with styles and scripts to enqueue
	 */
	function nexio_enqueue_optimized_responsive( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && nexio_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
...
if ( ! function_exists( 'nexio_get_date' ) ) {
	/**
	 ...
	 */
	function nexio_get_date( $dt = '', $format = '' ) {
		if ( '' == $dt ) {
			$dt = get_the_time( 'U' );
		}
		if ( date( 'U' ) - $dt > intval( nexio_get_theme_option( 'time_diff_before' ) ) * 24 * 3600 ) {
			$dt = date_i18n( '' == $format ? get_option( 'date_format' ) : $format, $dt );
		} else {
			// Translators: Add the human-friendly date difference
			$dt = sprintf( esc_html__( '%s ago', 'nexio' ), human_time_diff( $dt, current_time( 'timestamp' ) ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'nexio_get_privacy_text' ) ) {
	...
	function nexio_get_privacy_text() {
		...
		$privacy_text = nexio_get_theme_option( 'privacy_text' );
		return apply_filters(
			'nexio_filter_privacy_text',
			wp_kses(
				$privacy_text
				. ( ! empty( $page ) && ! empty( $privacy_text )
					// Translators: Add url to the Privacy Policy page
					? ' ' . sprintf( __( 'For further details on handling user data, see our %s.', 'nexio' ),
						'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
						. __( 'Privacy Policy', 'nexio' )
						. '</a>' )
					: ''
					),
				'nexio_kses_content'
...
if ( ! function_exists( 'nexio_verify_nonce' ) ) {
	 ...
	 */
	function nexio_verify_nonce( $nonce = 'nonce', $mask = '' ) {
		if ( empty( $mask ) ) {
...
if ( ! function_exists( 'nexio_get_upgrade_url' ) ) {
	...
	function nexio_get_upgrade_url( $params = array() ) {
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			...
			$default['domain']     = nexio_remove_protocol_from_url( get_home_url(), true );
