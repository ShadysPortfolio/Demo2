...
if ( ! function_exists( 'nexio_array_get_first' ) ) {
	/**
	 ...
	 */
	function nexio_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'nexio_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function nexio_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'nexio_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function nexio_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'nexio_array_from_list' ) ) {
	/**
	 ...
	 */
	function nexio_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'nexio_array_slice' ) ) {
	/**
	 ...
	 */
	function nexio_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'nexio_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * nexio_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * nexio_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function nexio_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'nexio_array_insert_after' ) ) {
	/**
	 ...
	 */
	function nexio_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = nexio_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: nexio_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'nexio_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function nexio_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'nexio_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function nexio_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'nexio_generate_id' ) ) {
	/**
	 ...
	 */
	function nexio_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'nexio_get_tag' ) ) {
	/**
	 ...
	 */
	function nexio_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'nexio_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function nexio_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'nexio_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function nexio_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( nexio_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'nexio_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function nexio_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'nexio_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function nexio_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'nexio_get_column_class' ) ) {
	/**
	 ...
	 */
	function nexio_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'nexio_stripslashes' ) ) {
	/**
	 ...
	 */
	function nexio_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'nexio_get_value_gp' ) ) {
	/**
	 ...
	 */
	function nexio_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'nexio_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function nexio_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'nexio_get_value_gps' ) ) {
	/**
	 ...
	 */
	function nexio_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'nexio_get_session_value' ) ) {
	/**
	 ...
	 */
	function nexio_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'nexio_set_session_value' ) ) {
	/**
	 ...
	 */
	function nexio_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'nexio_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function nexio_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'nexio_get_cookie' ) ) {
	/**
	 ...
	 */
	function nexio_get_cookie( $name, $defa = '' ) {
		return nexio_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'nexio_set_cookie' ) ) {
	/**
	 ...
	 */
	function nexio_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'nexio_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//nexio_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//nexio_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'nexio' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'nexio_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'nexio_hex2rgba' ) ) {
	/**
	 ...
	 */
	function nexio_hex2rgba( $hex, $alpha ) {
		$rgb = nexio_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'nexio_hex2hsb' ) ) {
	/**
	 ...
	 */
	function nexio_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = nexio_rgb2hsb( nexio_hex2rgb( $hex ) );
...
if ( ! function_exists( 'nexio_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'nexio_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'nexio_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'nexio_strshort' ) ) {
	 ...
	 */
	function nexio_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
...
if ( ! function_exists( 'nexio_strwords' ) ) {
	 ...
	 * @return string            A truncated string.
	 */
	function nexio_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'nexio_excerpt' ) ) {
	 ...
	 */
	function nexio_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
...
if ( ! function_exists( 'nexio_add_thumb_size' ) ) {
	...
	function nexio_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = nexio_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );
		...
		if ( $check_exists ) {
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = nexio_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = nexio_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'nexio_get_thumb_size' ) ) {
	/**
...
if ( ! function_exists( 'nexio_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function nexio_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = nexio_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = nexio_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'nexio_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'nexio_get_post_video_list' ) ) {
	/**
	...
	function nexio_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = nexio_add_to_url( $video, nexio_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'nexio_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'nexio_filter_video_width', 325, 'tiktok' ) : nexio_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'nexio_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'nexio_get_embed_video_tag_name' ) ) {
	/**
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = nexio_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										nexio_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'nexio_is_from_uploads' ) ) {
	/**
...
if ( ! function_exists( 'nexio_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'nexio_init_filesystem', 0 );
	/**
	 ...
	 */
	function nexio_init_filesystem( $force = false ) {
			...
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'nexio' ), $file ) );
					}
				}
			}
		}
	...
	function nexio_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? nexio_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'nexio' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'nexio' ), $file ) );
					}
				}
			}
		}
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'nexio' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'nexio' ), $path ) );
					}
				}
			}
		}
		...
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( nexio_is_on( nexio_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'nexio' ), $path ) );
					}
				}
			}
		}
	...
	function nexio_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = nexio_prepare_path( $src );
				$dst = nexio_prepare_path( $dst );
...
if ( ! function_exists( 'nexio_merge_css' ) ) {
	 ...
	 */
	function nexio_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
				...
				if ( $need_responsive ) {
					...
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'nexio_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'nexio_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'nexio' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = nexio_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			nexio_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'nexio') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'nexio_filter_css_output', apply_filters( 'nexio_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'nexio_filter_merge_css' ) ) {
...
if ( ! function_exists( 'nexio_is_external_url' ) ) {
	...
	function nexio_is_external_url( $url ) {
		return nexio_is_url( $url ) && strpos( $url, nexio_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}
