...
if ( ! defined( 'NEXIO_THEME_DIR' ) ) {
	define( 'NEXIO_THEME_DIR', trailingslashit( get_template_directory() ) );
}
if ( ! defined( 'NEXIO_THEME_URL' ) ) {
	define( 'NEXIO_THEME_URL', trailingslashit( get_template_directory_uri() ) );
}
if ( ! defined( 'NEXIO_CHILD_DIR' ) ) {
	define( 'NEXIO_CHILD_DIR', trailingslashit( get_stylesheet_directory() ) );
}
if ( ! defined( 'NEXIO_CHILD_URL' ) ) {
	define( 'NEXIO_CHILD_URL', trailingslashit( get_stylesheet_directory_uri() ) );
}
...
if ( ! function_exists( 'nexio_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'nexio_theme_setup1', 1 );
	/**
	 ...
	 */
	function nexio_theme_setup1() {
		// Make theme available for translation
		// Translations can be filed in the /languages directory
		// Attention! Translations must be loaded before first call any translation functions!
		load_theme_textdomain( 'nexio', nexio_get_folder_dir( 'languages' ) );
	}
}

if ( ! function_exists( 'nexio_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'nexio_theme_setup9', 9 );
	/**
	 * A general theme setup: add a theme supports, navigation menus, hooks for other actions and filters.
	 */
	function nexio_theme_setup9() {

		// Set theme content width
		$GLOBALS['content_width'] = apply_filters( 'nexio_filter_content_width', nexio_get_theme_option( 'page_width' ) );
		...
		register_nav_menus(
			array(
				'menu_main'   => esc_html__( 'Main Menu', 'nexio' ),
				'menu_mobile' => esc_html__( 'Mobile Menu', 'nexio' ),
				'menu_footer' => esc_html__( 'Footer Menu', 'nexio' ),
			)
		);
		...
		$mult        = nexio_get_theme_option( 'retina_ready', 1 );
		if ( $mult > 1 ) {
			$GLOBALS['content_width'] = apply_filters( 'nexio_filter_content_width', 1170 * $mult );
		}
		...
		// Add new thumb names
		add_filter( 'image_size_names_choose', 'nexio_theme_thumbs_sizes' );

		// Excerpt filters
		add_filter( 'excerpt_length', 'nexio_excerpt_length' );
		add_filter( 'excerpt_more', 'nexio_excerpt_more' );

		// Comment form
		add_filter( 'comment_form_fields', 'nexio_comment_form_fields' );
		add_filter( 'comment_form_fields', 'nexio_comment_form_agree', 11 );

		// Add required meta tags in the head
		add_action( 'wp_head', 'nexio_wp_head', 0 );

		// Load current page/post customization (if present)
		add_action( 'wp_footer', 'nexio_wp_footer' );
		add_action( 'admin_footer', 'nexio_wp_footer' );

		// Enqueue scripts and styles for the frontend
		add_action( 'wp_enqueue_scripts', 'nexio_load_theme_fonts', 0 );
		add_action( 'wp_enqueue_scripts', 'nexio_load_theme_icons', 0 );
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles', 1000 );                  // priority 1000 - load main theme styles
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_single', 1020);            // priority 1020 - load styles of single posts
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_plugins', 1100 );          // priority 1100 - load styles of the supported plugins
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_custom', 1200 );           // priority 1200 - load styles with custom fonts and colors
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_child', 1500 );            // priority 1500 - load styles of the child theme
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_responsive', 2000 );       // priority 2000 - load responsive styles after all other styles
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_single_responsive', 2020); // priority 2020 - load responsive styles of single posts after all other styles
		add_action( 'wp_enqueue_scripts', 'nexio_wp_styles_responsive_child', 2500);  // priority 2500 - load responsive styles of the child theme after all other responsive styles

		// Enqueue scripts for the frontend
		add_action( 'wp_enqueue_scripts', 'nexio_wp_scripts', 1000 );                 // priority 1000 - load main theme scripts
		add_action( 'wp_footer', 'nexio_localize_scripts' );

		// Add body classes
		add_filter( 'body_class', 'nexio_add_body_classes' );

		// Register sidebars
		add_action( 'widgets_init', 'nexio_register_sidebars' );
	}
}
...
if ( ! function_exists( 'nexio_wp_styles_single' ) ) {
	...
	function nexio_wp_styles_single() {
		if ( apply_filters( 'nexio_filters_separate_single_styles', false )
			&& apply_filters( 'nexio_filters_load_single_styles', nexio_is_single() || nexio_is_singular( 'attachment' ) || (int) nexio_get_theme_option( 'open_full_post_in_blog' ) > 0 )
		) {
...
if ( ! function_exists( 'nexio_wp_scripts' ) ) {
	...
	function nexio_wp_scripts() {
		$blog_archive = nexio_storage_get( 'blog_archive' ) === true || is_home();
		$blog_style   = nexio_get_theme_option( 'blog_style' );
		$use_masonry  = false;
		if ( strpos( $blog_style, 'blog-custom-' ) === 0 ) {
			$blog_id   = nexio_get_custom_blog_id( $blog_style );
			$blog_meta = nexio_get_custom_layout_meta( $blog_id );
			if ( ! empty( $blog_meta['scripts_required'] ) && ! nexio_is_off( $blog_meta['scripts_required'] ) ) {
			...
			$use_masonry = nexio_is_blog_style_use_masonry( $blog_style );
		}

		// Superfish Menu
		// Attention! To prevent duplicate this script in the plugin and in the menu, don't merge it!
		wp_enqueue_script( 'superfish', nexio_get_file_url( 'js/superfish/superfish.min.js' ), array( 'jquery' ), null, true );

		// Background video
		$header_video = nexio_get_header_video();
		if ( ! empty( $header_video ) && ! nexio_is_inherit( $header_video ) ) {
			if ( nexio_is_youtube_url( $header_video ) ) {
				wp_enqueue_script( 'jquery-tubular', nexio_get_file_url( 'js/tubular/jquery.tubular.js' ), array( 'jquery' ), null, true );
			} else {
				wp_enqueue_script( 'bideo', nexio_get_file_url( 'js/bideo/bideo.js' ), array(), null, true );
			}
		}

		// Merged scripts
		if ( nexio_is_off( nexio_get_theme_option( 'debug_mode' ) ) ) {
			wp_enqueue_script( 'nexio-init', nexio_get_file_url( 'js/__scripts' . ( nexio_is_preview() || ! nexio_optimize_css_and_js_loading() ? '-full' : '' ) . '.js' ), apply_filters( 'nexio_filter_script_deps', array( 'jquery' ) ), null, true );
		} else {
			// Skip link focus
			wp_enqueue_script( 'skip-link-focus-fix', nexio_get_file_url( 'js/skip-link-focus-fix/skip-link-focus-fix.js' ), null, true );
			// Theme scripts
			wp_enqueue_script( 'nexio-utils', nexio_get_file_url( 'js/utils.js' ), array( 'jquery' ), null, true );
			wp_enqueue_script( 'nexio-init', nexio_get_file_url( 'js/init.js' ), array( 'jquery' ), null, true );
		}
		...
		if ( nexio_get_theme_option( 'single_parallax' ) != 0
			&& ( nexio_is_singular( 'post' )
				|| ( (int) nexio_get_theme_option( 'open_full_post_in_blog' ) > 0 && ( is_home() || is_archive() || is_category() ) )
				)
		) {
			nexio_load_parallax_scripts();
		}

		// Load masonry scripts
		if ( ( $blog_archive && $use_masonry ) || ( nexio_is_singular( 'post' ) && str_replace( 'post-format-', '', get_post_format() ) == 'gallery' ) ) {
			nexio_load_masonry_scripts();
		}

		// Load tabs to show filters
		if ( $blog_archive && ! is_customize_preview() && ! nexio_is_off( nexio_get_theme_option( 'show_filters' ) ) ) {
			wp_enqueue_script( 'jquery-ui-tabs', false, array( 'jquery', 'jquery-ui-core' ), null, true );
		}

		// Comments
		if ( nexio_is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
			wp_enqueue_script( 'comment-reply' );
		}

		// Media elements library
		if ( nexio_get_theme_setting( 'use_mediaelements' ) ) {
...
if ( ! function_exists( 'nexio_localize_scripts' ) ) {
	...
	function nexio_localize_scripts() {
		...
		wp_localize_script( 'nexio-init', 'NEXIO_STORAGE', apply_filters( 'nexio_filter_localize_script', array(
			...
			// Sidemenu options
			'menu_side_stretch'   => (int) nexio_get_theme_option( 'menu_side_stretch' ) > 0,
			'menu_side_icons'     => (int) nexio_get_theme_option( 'menu_side_icons' ) > 0,
			...
			'open_full_post'      => (int) nexio_get_theme_option( 'open_full_post_in_blog' ) > 0,

			// Which block to load in the single posts
			'which_block_load'    => nexio_get_theme_option( 'posts_navigation_scroll_which_block' ),
...
if ( ! function_exists( 'nexio_add_body_classes' ) ) {
	 ...
	 */
	function nexio_add_body_classes( $classes ) {

		$classes[] = 'scheme_' . esc_attr( nexio_get_theme_option( 'color_scheme' ) );
		...
		$classes[] = 'body_style_' . esc_attr( nexio_get_theme_option( 'body_style' ) );

		if ( in_array( $blog_mode, array( 'post', 'page' ) ) || apply_filters( 'nexio_filter_single_post_header', nexio_is_singular( 'post' ) ) ) {
			$classes[] = 'is_single';
		} else {
			$classes[] = ' is_stream';
			$classes[] = 'blog_style_' . esc_attr( nexio_get_theme_option( 'blog_style' ) );
			if ( nexio_storage_get( 'blog_template' ) > 0 ) {
				$classes[] = 'blog_template';
			}
		}

		if ( apply_filters( 'nexio_filter_single_post_header', nexio_is_singular( 'post' ) || nexio_is_singular( 'attachment' ) ) ) {
			$classes[] = 'single_style_' . esc_attr( nexio_get_theme_option( 'single_style' ) );
		}

		if ( nexio_sidebar_present() ) {
			$classes[] = 'sidebar_show sidebar_' . esc_attr( nexio_get_theme_option( 'sidebar_position' ) );
			$classes[] = 'sidebar_small_screen_' . esc_attr( nexio_get_theme_option( 'sidebar_position_ss' ) );
		} else {
			$expand = nexio_get_theme_option( 'expand_content' );
			...
			if ( 'narrow' == $expand && ! nexio_is_singular( apply_filters('nexio_filter_is_singular_type', array('post') ) ) ) {
				$expand = 'normal';
			}
		...
		if ( nexio_is_on( nexio_get_theme_option( 'remove_margins' ) ) ) {
			$classes[] = 'remove_margins';
		}

		$bg_image = nexio_get_theme_option( 'front_page_bg_image' );
		if ( is_front_page() && ! empty( $bg_image ) && nexio_is_on( nexio_get_theme_option( 'front_page_enabled', false ) ) ) {
			$classes[] = 'with_bg_image';
		}

		$classes[] = 'trx_addons_' . esc_attr( nexio_exists_trx_addons() ? 'present' : 'absent' );

		$classes[] = 'header_type_' . esc_attr( nexio_get_theme_option( 'header_type' ) );
		$classes[] = 'header_style_' . esc_attr( 'default' == nexio_get_theme_option( 'header_type' ) ? 'header-default' : nexio_get_theme_option( 'header_style' ) );
		$header_position = nexio_get_theme_option( 'header_position' );
		if ( 'over' == $header_position && nexio_is_single() && ! has_post_thumbnail() ) {
			$header_position = 'default';
		}
		$classes[] = 'header_position_' . esc_attr( $header_position );

		$menu_side = nexio_get_theme_option( 'menu_side' );
		$classes[] = 'menu_side_' . esc_attr( $menu_side ) . ( in_array( $menu_side, array( 'left', 'right' ) ) ? ' menu_side_present' : '' );
		$classes[] = 'no_layout';

		if ( nexio_get_theme_setting( 'fixed_blocks_sticky' ) ) {
...
if ( ! function_exists( 'nexio_wp_footer' ) ) {
	/**
	...
	function nexio_wp_footer() {
		// Add header zoom
		$header_zoom = max( 0.2, min( 2, (float) nexio_get_theme_option( 'header_zoom' ) ) );
		if ( 1 != $header_zoom ) {
			nexio_add_inline_css( ".sc_layouts_title_title{font-size:{$header_zoom}em}" );
		}
		// Add logo zoom
		$logo_zoom = max( 0.2, min( 2, (float) nexio_get_theme_option( 'logo_zoom' ) ) );
		if ( 1 != $logo_zoom ) {
		...
		if ( ! empty( $css ) ) {
			wp_enqueue_style( 'nexio-inline-styles', nexio_get_file_url( 'css/__inline.css' ), array(), null );
			wp_add_inline_style( 'nexio-inline-styles', $css );
		}
	}
}
...
if ( ! function_exists( 'nexio_theme_fonts_links' ) ) {
	...
	function nexio_theme_fonts_links( $separate = false ) {
		...
		*/
		$google_fonts_enabled = ( 'off'  !== _x( 'on', 'Google fonts: on or off', 'nexio' ) );
		$google_fonts_api     = ( 'css2' !== _x( 'css2', 'Google fonts API: css or css2', 'nexio' ) ? 'css' : 'css2' );
		$adobe_fonts_enabled  = ( 'off'  !== _x( 'on', 'Adobe fonts: on or off', 'nexio' ) );
		$custom_fonts_enabled = ( 'off'  !== _x( 'on', 'Custom fonts (included in the theme): on or off', 'nexio' ) );

		if ( ( $google_fonts_enabled || $adobe_fonts_enabled || $custom_fonts_enabled ) && ! nexio_storage_empty( 'load_fonts' ) ) {
			$load_fonts = nexio_storage_get( 'load_fonts' );
			if ( count( $load_fonts ) > 0 ) {
				$google_fonts_subset = nexio_get_theme_option( 'load_fonts_subset' );
				$google_fonts = '';
				$adobe_fonts  = '';
				foreach ( $load_fonts as $font ) {
					$used = false;
					$slug = nexio_get_load_fonts_slug( $font['name'] );
					// Custom (in the theme folder included) font
					if ( $custom_fonts_enabled && empty( $font['styles'] ) && empty( $font['link'] ) ) {
						$url = nexio_get_file_url( "css/font-face/{$slug}/stylesheet.css" );
						if ( ! empty( $url ) ) {
							$links[ $slug ] = $url;
							$used = true;
						}
		...
		return apply_filters( 'nexio_filter_theme_fonts_links', $links );
	}
}

if ( ! function_exists( 'nexio_theme_fonts_for_editor' ) ) {
	/**
...
if ( ! function_exists( 'nexio_theme_thumbs_sizes' ) ) {
	 ...
	 * @return mixed
	 */
	function nexio_theme_thumbs_sizes( $sizes ) {
		$thumb_sizes = nexio_storage_get( 'theme_thumbs' );
		$mult        = nexio_get_theme_option( 'retina_ready', 1 );
		foreach ( $thumb_sizes as $k => $v ) {
			$sizes[ $k ] = $v['title'];
			if ( $mult > 1 ) {
				$sizes[ $k . '-@retina' ] = $v['title'] . ' ' . esc_html__( '@2x', 'nexio' );
			}
		}
		return $sizes;
	}
...
require_once NEXIO_THEME_DIR . 'includes/utils.php';
require_once NEXIO_THEME_DIR . 'includes/storage.php';

require_once NEXIO_THEME_DIR . 'includes/lists.php';
require_once NEXIO_THEME_DIR . 'includes/wp.php';

if ( is_admin() ) {
	require_once NEXIO_THEME_DIR . 'includes/tgmpa/class-tgm-plugin-activation.php';
	require_once NEXIO_THEME_DIR . 'includes/admin.php';
}

require_once NEXIO_THEME_DIR . 'theme-options/theme-customizer.php';

require_once NEXIO_THEME_DIR . 'front-page/front-page-options.php';

// Load a skins support
if ( defined( 'NEXIO_ALLOW_SKINS' ) && NEXIO_ALLOW_SKINS && file_exists( NEXIO_THEME_DIR . 'skins/skins.php' ) ) {
	require_once NEXIO_THEME_DIR . 'skins/skins.php';
}

// Load next files after the skins support loaded to allow a file substitution from the skins folder
require_once nexio_get_file_dir( 'theme-specific/theme-tags.php' );
require_once nexio_get_file_dir( 'theme-specific/theme-about/theme-about.php' );

// Add a free theme support
if ( NEXIO_THEME_FREE ) {
	require_once nexio_get_file_dir( 'theme-specific/theme-about/theme-upgrade.php' );
}

// Load an image hover effects
$nexio_file_dir = nexio_get_file_dir( 'theme-specific/theme-hovers/theme-hovers.php' );
if ( ! empty( $nexio_file_dir ) ) {
	require_once nexio_get_file_dir( 'theme-specific/theme-hovers/theme-hovers.php' );      // Substitution from skin is allowed
}

// Load a plugins support
$nexio_required_plugins = apply_filters( 'nexio_required_plugins', nexio_storage_get( 'required_plugins' ) );
if ( is_array( $nexio_required_plugins ) ) {
	foreach ( $nexio_required_plugins as $nexio_plugin_slug => $nexio_plugin_data ) {
		$nexio_plugin_slug = nexio_esc( $nexio_plugin_slug );
		$nexio_plugin_path = nexio_get_file_dir( sprintf( 'plugins/%1$s/%1$s.php', $nexio_plugin_slug ) );
		if ( ! empty( $nexio_plugin_path ) ) {
			require_once $nexio_plugin_path;
		}
	}
}
