...
if ( ! function_exists( 'nexio_gutenberg_fse_list_header_styles' ) ) {
	add_filter( 'nexio_filter_list_header_styles', 'nexio_gutenberg_fse_list_header_styles');
	/**
	 * Add a WordPress FSE templates to the headers list.
	 * 
	 * Hooks: add_filter( 'nexio_filter_list_header_styles', 'nexio_gutenberg_fse_list_header_styles');
	 * 
	 * @param array $list  Optional. An array with a list of headers from other available Page Builders.
	 */
	function nexio_gutenberg_fse_list_header_styles( $list = array() ) {
			...
			false, array(
				...
				'tax_query'    => array(
										...
										array(
											...
											'field'    => 'name',
											'terms'    => NEXIO_FSE_TEMPLATE_PART_AREA_HEADER,
										)
									)
			)
		);
		...
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
			foreach ( $data['templateParts'] as $template ) {
				if ( ! empty( $template['area'] )
					&& NEXIO_FSE_TEMPLATE_PART_AREA_HEADER == $template['area']
...
if ( ! function_exists( 'nexio_gutenberg_fse_list_footer_styles' ) ) {
	add_filter( 'nexio_filter_list_footer_styles', 'nexio_gutenberg_fse_list_footer_styles');
	/**
...
if ( ! function_exists( 'nexio_gutenberg_fse_list_sidebar_styles' ) ) {
	 ...
	 */
	function nexio_gutenberg_fse_list_sidebar_styles( $list = array() ) {
		if ( ! nexio_gutenberg_is_fse_theme() ) {
			return $list;
		}
		$new_list = array();
			...
			false, array(
				...
				'tax_query'    => array(
										...
										array(
											'taxonomy' => 'wp_template_part_area',
											'field'    => 'name',
											'terms'    => NEXIO_FSE_TEMPLATE_PART_AREA_SIDEBAR,
										)
									)
			)
		);
		...
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
			foreach ( $data['templateParts'] as $template ) {
				if ( ! empty( $template['area'] )
					&& NEXIO_FSE_TEMPLATE_PART_AREA_SIDEBAR == $template['area']
					&& ! in_array( $template['title'], $new_list )
				) {
