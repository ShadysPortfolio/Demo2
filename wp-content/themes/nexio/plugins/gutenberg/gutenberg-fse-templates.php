...
if ( ! function_exists( 'nexio_gutenberg_fse_replace_frontpage_content' ) ) {
	add_filter( 'frontpage_template', 'nexio_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'frontpage_template', 'nexio_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	 *
	 * @param string   $template  Path to the template. See locate_template().
	 * @param string   $type      Sanitized filename without extension.
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function nexio_gutenberg_fse_replace_frontpage_content( $template, $type = '', $templates = array() ) {
...
if ( ! function_exists( 'nexio_gutenberg_fse_replace_no_posts_content' ) ) {
	add_filter( 'search_template',  'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'index_template',   'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'archive_template', 'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	/**
	 * Substitute a content of the global variable $_wp_current_template_content
	 * with a content of the template 'parts/none-archive.html' or 'templates/none-archive.html'.
	 * 
	 * Hooks: add_filter( 'search_template',  'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'index_template',   'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'archive_template', 'nexio_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 ...
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function nexio_gutenberg_fse_replace_no_posts_content( $template, $type = '', $templates = array() ) {
		if ( substr( $template, -19 ) == 'template-canvas.php' ) {
			if ( ! have_posts() ) {
				...
				$no_posts_file = nexio_get_file_dir( ( $part_exists ? 'parts/' : 'templates/' ) . "{$part_name}.html" );
				if ( ! empty( $no_posts_file ) ) {
					// Check if exists a saved template with a same name
					$posts = get_posts( array(
						'name'           => $part_name,
						'posts_per_page' => 1,
						'post_type'      => defined( 'NEXIO_FSE_TEMPLATE_PART_PT' ) ? NEXIO_FSE_TEMPLATE_PART_PT : 'wp_template_part',
					) );
					// Get a template content from the database or from the file (if a saved version is not found)
					$content = ! empty( $posts[0]->post_content ) ? $posts[0]->post_content : nexio_fgc( $no_posts_file );
...
if ( ! function_exists( 'nexio_gutenberg_fse_show_template_as_layout' ) ) {
	add_filter( 'nexio_filter_custom_layout_shown', 'nexio_gutenberg_fse_show_template_as_layout', 10, 3 );
	/**
	 * Show an FSE template (header/footer/sidebar) inside a non-FSE templates.
	 * 
...
if ( ! function_exists( 'nexio_gutenberg_fse_modify_template_replace_header' ) ) {
	...
	function nexio_gutenberg_fse_modify_template_replace_header( $content, $template, $type, $templates ) {
		...
		if ( 'custom' == $header_type ) {
			$header_style = nexio_get_theme_option( "header_style" );
			...
			if ( strpos( $header_style, "header-fse-template-" ) !== false ) {
				...
				ob_start();
				do_action( 'nexio_action_fse_before_custom_header', $content, $template, $type, $templates );
				...
				ob_start();
				do_action( 'nexio_action_fse_after_custom_header', $content, $template, $type, $templates );
			...
			// Custom header's layout
				...
				do_action( 'nexio_action_fse_before_custom_header', $content, $template, $type, $templates );
				// Custom header
				get_template_part( apply_filters( 'nexio_filter_get_template_part', "templates/header-custom" ) );
				// Trigger action after the custom header to allow other modules include an additional content to the custom header
				do_action( 'nexio_action_fse_after_custom_header', $content, $template, $type, $templates );
				// Get output
...
if ( ! function_exists( 'nexio_gutenberg_fse_custom_header_add_side_menu' ) ) {
	add_action( 'nexio_action_fse_after_custom_header', 'nexio_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	/**
		...
		if ( nexio_sidebar_present() ) {
			...
			if ( nexio_exists_trx_addons() ) {
				...
				// A custom sidebar (built with FSE or with any other builder)
					...
					$sidebar_style = nexio_get_theme_option( "sidebar_style" );
					$sidebar_id = nexio_get_custom_sidebar_id();
					...
					// Custom sidebar's layout
						...
						do_action( 'nexio_action_fse_before_custom_sidebar', $content, $template, $type, $templates );
						// Custom sidebar
						do_action( 'nexio_action_show_layout', $sidebar_id );
						// Trigger action after th custom sidebar to allow other modules include an additional content to the custom sidebar
						do_action( 'nexio_action_fse_after_custom_sidebar', $content, $template, $type, $templates );
						// Get output
						$html = ob_get_contents();
						ob_end_clean();
						if ( ! empty( $html ) ) {
...
// Footer
//----------------------------------------------
if ( ! function_exists( 'nexio_gutenberg_fse_modify_template_replace_footer' ) ) {
	add_filter( 'nexio_filter_wp_current_template_content', 'nexio_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	/**
	...
	function nexio_gutenberg_fse_featured_image_renderer( $attributes, $content, $block ) {
		...
		nexio_show_post_featured( apply_filters( 'nexio_filter_args_featured',
			array(
				...
				'thumb_size' => nexio_get_thumb_size( strpos( nexio_get_theme_option( 'body_style' ), 'full' ) !== false
									? 'full'
