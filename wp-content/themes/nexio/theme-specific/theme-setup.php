...
if ( ! defined( 'NEXIO_THEME_FREE' ) ) {
	define( 'NEXIO_THEME_FREE', false );
}
if ( ! defined( 'NEXIO_THEME_FREE_WP' ) ) {
	define( 'NEXIO_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'NEXIO_THEME_ALLOW_UPDATE' ) ) {
	define( 'NEXIO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'NEXIO_ALLOW_SKINS' ) ) {
	define( 'NEXIO_ALLOW_SKINS', true );
}
if ( ! defined( 'NEXIO_DEFAULT_SKIN' ) ) {
	define( 'NEXIO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'NEXIO_REMEMBER_SKIN' ) ) {
	define( 'NEXIO_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'nexio_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function nexio_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = nexio_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'nexio_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'nexio_trx_updater_get_theme_info' );
	/**
...
if ( ! function_exists( 'nexio_customizer_theme_setup1' ) ) {
	 ...
	 */
	function nexio_customizer_theme_setup1() {
		nexio_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'nexio_filter_allow_fullscreen' return false)
