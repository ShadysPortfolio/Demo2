<div class="front_page_section front_page_section_features<?php
	$nexio_scheme = nexio_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $nexio_scheme ) && ! nexio_is_inherit( $nexio_scheme ) ) {
		echo ' scheme_' . esc_attr( $nexio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( nexio_get_theme_option( 'front_page_features_paddings' ) );
	if ( nexio_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$nexio_css      = '';
		$nexio_bg_image = nexio_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$nexio_anchor_icon = nexio_get_theme_option( 'front_page_features_anchor_icon' );
	$nexio_anchor_text = nexio_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( nexio_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$nexio_css      = '';
			$nexio_bg_mask  = nexio_get_theme_option( 'front_page_features_bg_mask' );
			$nexio_bg_color_type = nexio_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $nexio_bg_color_type ) {
				$nexio_bg_color = nexio_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$nexio_caption = nexio_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $nexio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $nexio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $nexio_caption, 'nexio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$nexio_description = nexio_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $nexio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $nexio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $nexio_description ), 'nexio_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! nexio_exists_trx_addons() ) {
						...
						nexio_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
