<div class="front_page_section front_page_section_blog<?php
	$nexio_scheme = nexio_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $nexio_scheme ) && ! nexio_is_inherit( $nexio_scheme ) ) {
		echo ' scheme_' . esc_attr( $nexio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( nexio_get_theme_option( 'front_page_blog_paddings' ) );
	if ( nexio_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$nexio_css      = '';
		$nexio_bg_image = nexio_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$nexio_anchor_icon = nexio_get_theme_option( 'front_page_blog_anchor_icon' );
	$nexio_anchor_text = nexio_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( nexio_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$nexio_css      = '';
			$nexio_bg_mask  = nexio_get_theme_option( 'front_page_blog_bg_mask' );
			$nexio_bg_color_type = nexio_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $nexio_bg_color_type ) {
				$nexio_bg_color = nexio_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$nexio_caption = nexio_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $nexio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $nexio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $nexio_caption, 'nexio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$nexio_description = nexio_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $nexio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $nexio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $nexio_description ), 'nexio_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! nexio_exists_trx_addons() ) {
						...
						nexio_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
